                  ...
                  , OSD_BITMAPINFO* pBmpInfo)
{
    ...
    {
		printf("fread OSD_BITMAPFILEHEADER failed:%s!\n", filename);
        fclose(pFile);
        return -1;
	}
    if (fread(pBmpInfo, 1, sizeof(OSD_BITMAPINFO), pFile) != sizeof(OSD_BITMAPINFO))
    {
		printf("fread OSD_BITMAPINFO failed:%s!\n", filename);
        fclose(pFile);
        return -1;
	}
    fclose(pFile);
...
int LoadBMP(const char* filename, OSD_LOGO_T* pVideoLogo)
{
    ...
    if (fread(pOrigBMPBuf, 1, (HI_U32)(h * stride), pFile) != (HI_U32)(h * stride) )
    {
